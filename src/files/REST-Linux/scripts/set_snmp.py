...
def setsnmp_init(parser, parser_list):
    ...
    ewr_help = '''whether read&write Community enabled'''
    sp.add_argument('-RWE', dest='RWCommunityEnabled',
                    type=str, required=False,
                    choices=['True', 'False'], help=ewr_help)
    roc_help = '''Read Only Community Name'''
    sp.add_argument('-ROC', dest='ReadOnlyCommunity',
                    type=str, required=False, help=roc_help)
    rwc_help = '''Read Write Community Name'''
    sp.add_argument('-RWC', dest='ReadWriteCommunity',
                    type=str, required=False, help=rwc_help)
    v3ap_help = '''SNMP V3 authentication protocol'''
    sp.add_argument('-V3AP', dest='SnmpV3AuthProtocol',
                    type=str, required=False,
                    choices=['MD5', 'SHA1'],
                    help=v3ap_help)
    v3ep_help = '''SNMP V3 encrypt protocol'''
    sp.add_argument('-V3EP', dest='SnmpV3PrivProtocol',
                    type=str, required=False,
                    choices=['DES', 'AES'],
                    help=v3ep_help)

    ts_help = '''whether trap notification service enabled'''
    ...
    tu_help = '''exists user for trap v3'''
...
def remove_none_values(trap):
    return dict((k, v) for k, v in trap.items() if v is not None)
