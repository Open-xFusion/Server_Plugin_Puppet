# -*- coding:utf-8 -*-
"""
Function: get_memory.py moudle. This moudle mainly involves the
 querying Memory Information function.
...
def get_memory(client, memory_url):
    ...
    if memory_resp['status_code'] == 200:
        ...
        print(MEM_FORMAT % ('DeviceLocator', key.get('DeviceLocator', None)))
        print(MEM_FORMAT % ('CapacityMiB', key.get('CapacityMiB', None)))
        print(MEM_FORMAT % ('Manufacturer', key.get('Manufacturer', None)))
        print(MEM_FORMAT %
              ('OperatingSpeedMhz', key.get('OperatingSpeedMhz', None)))
        print(MEM_FORMAT % ('SerialNumber', key.get('SerialNumber', None)))
        print(MEM_FORMAT %
              ('MemoryDeviceType', key.get('MemoryDeviceType', None)))
        print(MEM_FORMAT % ('DataWidthBits', key.get('DataWidthBits', None)))
        print(MEM_FORMAT % ('RankCount', key.get('RankCount', None)))
        print(MEM_FORMAT % ('PartNumber', key.get('PartNumber', None)))
        if key.get(common_function.OEM_KEY) is not None:
            oem_dict = key.get(common_function.OEM_KEY)
            vendor_dict = oem_dict.get(common_function.COMMON_KEY)
            for oem_key, value in vendor_dict.items():
                print(MEM_FORMAT % (oem_key, value))
        print('\n[MemoryLocation]')
        memory_socket = key.get('MemoryLocation', None)
        if memory_socket is not None:
            print(MEM_FORMAT % ("Socket", memory_socket.get("Socket", None)))
            print(MEM_FORMAT %
                  ("Controller", memory_socket.get("Controller", None)))
            print(MEM_FORMAT % ("Channel", memory_socket.get("Channel", None)))
            print(MEM_FORMAT % ("Slot", memory_socket.get("Slot", None)))
        print('\n[Status]')
...
def get_memory_sys(client):
    ...
    if sys_resp['status_code'] == 200:
        ...
        print(MEM_FORMAT % ('TotalSystemMemoryGiB',
                            sys_resp['resource']['MemorySummary'][
                                'TotalSystemMemoryGiB']))
        print('\n[Status]')
        print(MEM_FORMAT % ('HealthRollup',
                            sys_resp['resource']['MemorySummary']['Status'][
                                'HealthRollup']))
    return sys_resp


def getmemory(client, _):
    """
    #==========================================================================
