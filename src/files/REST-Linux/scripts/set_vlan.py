# -*- coding:utf-8 -*-
"""
Function: set_vlan.py moudle. This moudle mainly involves the
 setting VLAN Information function.
...
def package_request(args, payload):
    ...
    if args.VLANEnable is not None:
        vlan["VLANEnable"] = args.VLANEnable == str(True)
    payload["VLAN"] = vlan
...
def check_err_info(resp_vlan, code_vlan):
    """
    #==========================================================================
    # @Method: Determine whether all attributes are set successfully.
...
def check_parameter(parser, args):
    ...
    if args.VLANId is not None and (args.VLANId < 1 or args.VLANId > 4094):
        parser.error('argument -I: '
                     'invalid choice: %s (choose from 1 to 4094)' % args.VLANId)
