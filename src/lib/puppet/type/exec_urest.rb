module Puppet
  Type.newtype(:exec_urest) do
    include Puppet::Util::Execution
    require 'timeout'

    @doc = "Executes external commands.

      Any command in an `exec_urest` resource **must** be able to run multiple times
      ...
      * The exec has an `onlyif`, `unless`, or `creates` attribute, which prevents
        ...
        `onlyif` and `unless` commands of an `exec_urest` are used in the process of
        determining whether the `exec_urest` is already in sync, therefore they must be run
      ...
      * The exec has `refreshonly => true`, which allows Puppet to run the
      ...
      The state managed by an `exec_urest` resource represents whether the specified command
      ...
      **Duplication:** Even though `command` is the namevar, Puppet allows
      multiple `exec_urest` resources with the same `command` value.

      **Refresh:** `exec_urest` resources can respond to refresh events (via
    ...
    newparam(:command) do
      ...
      desc "The actual command to execute.  Must either be fully qualified
        ...
        Multiple `exec_urest` resources can use the same `command` value; Puppet
        only uses the resource title to ensure `exec_urest`s are unique.
    ...
    newparam(:refresh) do
      desc "An alternate command to run when the `exec_urest` receives a refresh event
    ...
    newcheck(:unless) do
      desc <<-'EOT'
        ...
        when the `exec_urest` can run. If present, Puppet runs this test command
        ...
        `exec_urest` is already in sync, it must be run during a noop Puppet run.
        ...
        This `exec_urest` would only run if every command in the array has a
        non-zero exit code.
      EOT
    ...
    newcheck(:onlyif) do
      desc <<-'EOT'
        ...
        This `exec_urest` would only run if every command in the array has an
